...
export default function CreateFlashcardsPage() {
  ...
  const [flashcards, setFlashcards] = useState<FlashcardItem[]>([
    { front: "", back: "", useFrontImage: false, useBackImage: false },
  ]);
  ...
  const updateFlashcard = (index: number, field: keyof FlashcardItem, value: any) => {
    const newFlashcards = [...flashcards];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate front side
    if (flashcards.some((card) => {
      if (card.useFrontImage) {
        return !card.frontImage;
      }
      return !card.front.trim();
    })) {
      setError(t("validation.frontRequired"));
      return;
    }

    // Validate back side
    if (flashcards.some((card) => {
      if (card.useBackImage) {
        return !card.backImage;
      }
      return !card.back.trim();
    })) {
      setError(t("validation.backRequired"));
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-6">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full py-3 border-2 border-dashed border-[var(--color-border)] rounded-lg text-[var(--color-muted)] hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] transition flex items-center justify-center gap-2"
              >
            ...
            >
              <button
                type="submit"
                disabled={isLoading}
                className="flex-1 py-4 px-6 bg-[var(--color-primary)] text-white font-semibold rounded-lg hover:bg-[var(--color-primary-dark)] transition disabled:opacity-50 text-lg"
              >
                {isLoading ? t("creation.creating") : t("creation.createSet")}
              </button>
            </motion.div>
          </form>
        </div>
